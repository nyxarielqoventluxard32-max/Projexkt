...
suspend fun bypass(mainUrl: String): String {
    val (savedCookie, savedTimestamp) = DisneyPlusStorage.getCookie()

    if (!savedCookie.isNullOrEmpty() &&
        System.currentTimeMillis() - savedTimestamp < 54_000_000
    ) {
        return savedCookie
    }
    ...
    val newCookie = try {
        ...
        DisneyPlusStorage.clearCookie()
        throw e
    }

    if (newCookie.isNotEmpty()) {
        DisneyPlusStorage.saveCookie(newCookie)
    }
...
suspend fun getVideoToken(
    ...
    val document = app.get(
        "$newUrl/play.php?id=$id&$h",
        headers = headers2
    ).document

    return document.select("body").attr("data-h")
}
